...
turnMethods = {
  ...
  center: function(page) {
    ...
    if (!data.noCenter) {
      ...
      if (data.page==1)
      {
        finalWidth = maxWidth/2 - size.width*0.75;
      }
      else if (data.page==data.totalPages)
      {
        finalWidth = maxWidth/2 - size.width*0.25;
      }
      else
      {
        finalWidth = maxWidth/2 - size.width/2;
      }

      var finalHeight = (maxHeight - size.height)/2;
      left = finalWidth;
      $(this).css({marginLeft: finalWidth, marginTop: finalHeight});
    }
  ...
  if (!data.wrapper)
    switch (data.effect) {
      case 'hard':
        ...
        if (turnData.opts.gradients) {
          data.ashadow = $('<div/>', divAtt(0, 0,  -2)).
            hide().
            appendTo(parent);

          data.bshadow = $('<div/>', divAtt(0, 0,  -2));
        }
